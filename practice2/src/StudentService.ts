...
import { createProduct } from './ProductReducer';
 ...
 export async function loadProduct(id: number, dispatch: DispatchFunc): Promise<Product> { 
    try { 
        const response = await axios.get("/products/" + id); return response.data; 
    }
    catch (error) { onError('조회 에러: ', error, dispatch); return createProduct(); } 
}
...
export async function deleteProduct(id: number, dispatch: DispatchFunc) { 
    try { 
        await axios.delete("/products/" + id); reloadProducts(dispatch); 
    } catch (error) { onError('삭제 에러: ', error, dispatch); }
}
